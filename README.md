...
对结果可客观判定、重复执行或回归风险高的工作流，先定义最小评测契约：目标案例、既有正确案例、边界不变量和验收证据。修改只有在目标改善且既有案例无回退时才接受；工具错误、重复调用和检索遗漏等轨迹异常用于定位原因，不单独决定成败。

新想法先登记在本表，不要求立即实现。同类失败累计两次，或同一工作流发生第三次规则修改时，固化为可重复评测案例。开放式科研写作和方案构思保持人工审阅，不使用机械 F1 驱动规则修改。

| 候选工作流 | 状态与触发条件 | 评测重点 |
| --- | --- | --- |
| AGENTS 同步与唯一 Hook | 首个试点，当前即可建立 | 源与镜像哈希一致、内容不变时无提交、始终只有一个 SessionStart Hook、隐藏运行、临时文件清理、只修改登记路径 |
| 子代理工作流 | 出现重复检索、漏检或错误结论时收集实例 | 检索覆盖、证据有效性、重复调用、工具错误、未覆盖项；轨迹信号只触发重派或复核 |
| 技能与插件清理 | 下次全局清理时启用 | 区分 installed、enabled、active、cache、shared；删除目标、共享依赖和保留边界逐项复核 |
| PPTX 只改文字 | 下次实际编辑时执行 | 目标 OOXML 文本变化，其他 ZIP 条目内容保持字节一致 |
| 科研写作与方案构思 | 不进入自动评测 | 由人工检查证据边界、语义、篇幅和文风 |

不新增全局 Hook、常驻评测服务或自动部署规则。需要自动化时，先在项目内手动运行评测；LLM 可以提出小型 diff，应用和全局部署仍需按风险等级确认。
